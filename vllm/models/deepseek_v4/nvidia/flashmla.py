...
def _dbg_stats(name: str, t: torch.Tensor | None) -> str:
    if t is None:
        return f"{name}=None"
    tf = t.float()
    return (
        f"{name}[mean={tf.abs().mean().item():.3e} max={tf.abs().max().item():.3e} "
        f"inf={int(tf.isinf().sum().item())} nan={int(tf.isnan().sum().item())} "
        f"min={tf.min().item():.3e} maxv={tf.max().item():.3e}]"
    )


def _dbg_layer0(layer: "DeepseekV4FlashMLAAttention") -> bool:
    import os

    return (
        os.environ.get("XIAOTU_DEBUG_L4") == "1"
        and layer.prefix.endswith("layers.0.attn")
    )
...
class DeepseekV4FlashMLAAttention(DeepseekV4Attention):
    ...
    def forward_mqa(
        ...
        if _dbg_layer0(self):
            print(
                f"[L4] forward_mqa out {_dbg_stats('output', output)}",
                flush=True,
            )
    ...
    def _forward_decode(
        ...
        if is_triton_sparse_mla_enabled(q.device):
            if _dbg_layer0(self):
                print(
                    f"[L4] decode enter {_dbg_stats('q', q)} swa_only={swa_only} "
                    f"compress_ratio={self.compress_ratio} "
                    f"{_dbg_stats('swa_indices', swa_indices)} "
                    f"{_dbg_stats('swa_lens', swa_lens)} "
                    f"{_dbg_stats('topk_indices', topk_indices)} "
                    f"{_dbg_stats('topk_lens', topk_lens)}",
                    flush=True,
                )
            if swa_only:
        ...
        if _dbg_layer0(self):
            print(
                f"[L4] decode out {_dbg_stats('output', output)}",
                flush=True,
            )
    ...
    def _forward_prefill(
        ...
        if _dbg_layer0(self):
            print(
                f"[L4] prefill enter {_dbg_stats('q', q)} "
                f"swa_cache_shape={tuple(swa_k_cache.shape)} "
                f"swa_cache_dtype={swa_k_cache.dtype} "
                f"top_k={top_k} swa_only={swa_only} num_prefills={num_prefills} "
                f"num_decodes={num_decodes} combined_topk={combined_topk} "
                f"triton={is_triton_sparse_mla_enabled(q.device)} "
                f"split={indexed_d512_split_prefill} chunked={indexed_d512_chunked_prefill}",
                flush=True,
            )
        for chunk_start, chunk_end, chunk_n, chunk_m in chunk_plan:
            ...
            if _dbg_layer0(self):
                print(
                    f"[L4] prefill chunk n={chunk_n} m={chunk_m} "
                    f"{_dbg_stats('kv_gathered', kv[:chunk_size])} "
                    f"{_dbg_stats('combined_lens', combined_lens.float())} "
                    f"{_dbg_stats('combined_indices', combined_indices.float())}",
                    flush=True,
                )
            if triton_sparse_mla_enabled:
            ...
            if _dbg_layer0(self):
                print(
                    f"[L4] prefill chunk out "
                    f"{_dbg_stats('output', output[query_start:query_end])}",
                    flush=True,
                )
